...
const fileExtensionConfig = require('../constants/file-extensions.json');
...
validationHelper.removeUrlLinks = (field) => {
  field = field.replaceAll('https://', '');
  field = field.replaceAll('http://', '');
  field = field.replaceAll('https', '');
  field = field.replaceAll('http', '');
  field = field.replaceAll('com', '');
...
/**
 * Validate if parameter is an object or an array
 * @param {string} inFuncName
 * @param {object} params
 */
...
/**
 * Validate if file extension is allowed to be uploaded
 * @param {string} fileExtension
 * @param {string} originalFileName
 * @returns {boolean}
 */
validationHelper.isAllowedFileExtension = (
  fileExtension = '',
  originalFileName = '',
) => {
  const BLACKLISTED_FILE_EXTENSIONS =
    fileExtensionConfig.BLACKLISTED_FILE_EXTENSIONS;

  if (!fileExtension && originalFileName.split('.').length < 2) {
    return false;
  }

  const extension = fileExtension || originalFileName.split('.').reverse()[0];

  return BLACKLISTED_FILE_EXTENSIONS.indexOf(extension.toLowerCase()) < 0;
};
