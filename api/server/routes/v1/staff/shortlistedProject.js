...
module.exports = (fastify, opts, next) => {
  fastify.route({
    method: 'POST',
    url: '/staff/shortlisted-project/:shortlisted_project_id/setting',
    schema: {
      params: {
        type: 'object',
      ...
      body: {
        ...
        properties: {
          ...
          shortlisted_property_settings: { type: 'array' },
    ...
    handler: async (req, res) => {
      ...
      const [err] = await h.database.transaction(async (transaction) => {
        nonStaffShortlistedProjectSettingService.setDbTransaction(transaction);
        return catchError(
          nonStaffShortlistedProjectSettingService.upsertShorlistedProjectSettings(
            req,
          ),
        );
      });
