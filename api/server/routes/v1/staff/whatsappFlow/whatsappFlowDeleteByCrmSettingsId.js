...
* }} param0 
*/
function _handleResponse ({req, res, whatsapp_flow_ids }){
...
async function deleteOrArchieveFlow ({
  flow_id,
  access_token,
  credentials,
}) {
  try {
    ...
    // currently no way to query if the whatsappFlow is archived or not
    await h.whatsapp.archiveWhatsappFlowById({
      flow_id,
      access_token,
      credentials,
    });
...
async function handler(req, res) {
  ...
  const log = req.log.child({
    route: '/v1/staff/whatsapp-flows/:crm_settings_id/delete-by-crm-settings-id',
    method: 'post',
    sub_id: uuidv4()
  });
  ...
  try {
    ...
    const oldWhatsappFlows = await c.whatsappFlow.findAll({
      crm_settings_fk: crm_settings_id
    });

    let whatsapp_flow_ids = [];

    for (const oldWhatsappFlow of oldWhatsappFlows) {
      const flow_id = oldWhatsappFlow.flow_id;
  
      await deleteOrArchieveFlow({
        flow_id,
        access_token,
        credentials: agencyBufferedTemplateCredentials,
      });
  
      await c.whatsappFlow.destroy({ whatsapp_flow_id: oldWhatsappFlow.whatsapp_flow_id });

      whatsapp_flow_ids.push(oldWhatsappFlow.whatsapp_flow_id)
    }

  
    _handleResponse({ req, res, whatsapp_flow_ids });
  } catch (err) {
