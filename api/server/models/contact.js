...
module.exports = (sequelize, DataTypes) => {
  class contact extends Model {
    ...
    static associate(models) {
      ...
      contact.hasMany(models.whatsapp_message_tracker, {
        foreignKey: 'contact_fk',
      });
      contact.hasMany(models.sms_message_tracker, {
        foreignKey: 'contact_fk',
      });
      contact.hasMany(models.whatsapp_chat, {
        foreignKey: 'contact_fk',
      });
      contact.belongsTo(models.agency_custom_landing_pages, {
        foreignKey: 'permalink_template',
        targetKey: 'landing_page_slug',
      });
      contact.hasMany(models.contact_list_user, {
        foreignKey: 'contact_id',
      });
      contact.hasMany(models.mindbody_client_visits, {
        foreignKey: 'contact_fk',
      });
      contact.hasMany(models.mindbody_client_contracts, {
        foreignKey: 'contact_fk',
      });
      contact.hasMany(models.live_chat, {
        foreignKey: 'contact_fk',
      });
      contact.hasMany(models.live_chat_session, {
        foreignKey: 'contact_fk',
      });
      contact.hasMany(models.mindbody_client_services, {
        foreignKey: 'contact_fk',
      });
      contact.hasMany(models.hubspot_form_submission_payload, {
        foreignKey: 'contact_fk',
      });
    }
  }
  let fields = {
    ...
    opt_out_whatsapp_date: { type: DataTypes.DATE },
    line_user_id: { type: DataTypes.STRING, allowNull: true },
    opt_out_line: {
      type: DataTypes.BOOLEAN,
      defaultValue: false,
    },
    opt_out_line_date: { type: DataTypes.DATE },
    messenger_id: { type: DataTypes.STRING, allowNull: true },
    opt_out_messenger: {
      type: DataTypes.BOOLEAN,
      defaultValue: false,
    },
    opt_out_messenger_date: { type: DataTypes.DATE },
    ...
    buy_status: { type: DataTypes.STRING }, // constant.BUY_STATUS
    subscriber_status: { type: DataTypes.STRING, allowNull: true },
    manual_label: { type: DataTypes.STRING, allowNull: true },
    ...
    whatsapp_engagement: { type: DataTypes.STRING },
    paused_automation: { type: DataTypes.BOOLEAN, defaultValue: false },
    ...
    from_export: { type: DataTypes.BOOLEAN },
    has_appointment: { type: DataTypes.BOOLEAN },
