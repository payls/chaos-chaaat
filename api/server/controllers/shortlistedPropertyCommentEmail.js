...
module.exports.makeShortlistedPropertyCommentEmailController = (models) => {
  ...
  const gmailIntegrationController =
    require('./gmailIntegrations').makeGmailIntegrationController(models);
      ...
      if (send_email) {
        ...
        if (!emailSent) {
          ...
          await h.email.sendEmail(
            `Pave <no-reply@${config?.email?.domain || 'chaaat.io'}>`,
  ...
  shortlistedPropertyCommentEmailController.constructShortlistPropertyEmail =
    async (
      ...
      if (
        ...
        if (send_email) {
          try {
            emailSent = await gmailIntegrationController.sendEmail({
              agency_user_fk: agency_user_id,
              email: buyerEmail,
              subject: custom_email_subject,
              body: custom_email_body,
            });
          } catch (error) {
            Sentry.captureException(error);
            console.log(`${funcName}: ${error}`);
