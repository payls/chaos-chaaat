...
module.exports.makeController = (models) => {
  const { automation_rule_trigger: model } = models;
  const ctr = {};
  ...
  ctr.create = async (record, { transaction } = {}) => {
    ...
    const rule_trigger_id = h.general.generateId();
    await model.create(
      {
        rule_trigger_id,
        description,
  ...
  ctr.update = async (rule_trigger_id, record, { transaction } = {}) => {
    ...
    await model.update(
      ...
      { where: { rule_trigger_id }, transaction },
    );
    return rule_trigger_id;
  };
