...
module.exports.makeProjectController = (models) => {
  ...
  projectController.findAll = async (
    where,
    { include, transaction, order, limit, offset } = {},
  ) => {
   ...
   * @returns {Promise<Array>}
   */
  projectController.count = async (where, { include, transaction } = {}) => {
    const funcName = 'projectController.count';
    h.validation.requiredParams(funcName, { where });
    h.validation.isObjectOrArray(funcName, { where });
    const records = await projectModel.count({
      where: { ...where },
      distinct: true,
      include,
      transaction,
