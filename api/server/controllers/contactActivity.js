...
const {
  makeShortListedPropertyCommentController,
} = require('./shortlistedPropertyComment');

const {
  makeEmailNotificationSettingController,
} = require('./emailNotificationSetting');
...
module.exports.makeContactActivityController = (models, fastify) => {
  ...
  /**
   * Log contact activity to Salesforce Direct integration
   * @param {{
   *  contact_fk: string,
   *  activity_type: string,
   *  activity_meta: string
   ...
   * @returns {Promise<void>}
   */
  contactActivityController.logActivityToSalesforceDirectIntegration = async (
    record,
    { transaction } = {},
    request = {},
  ) => {
    ...
    const funcName =
      'contactActivityController.logActivityToSalesforceDirectIntegration';
    try {
      h.validation.requiredParams(funcName, { record });
      const { contact_fk, activity_type, activity_meta } = record;
      ...
      const agency_id = contact.agency_fk;

      // check if there is agency oauth
      ...
      request.log.info(
        {
          label:
            'contactActivityController.logActivityToSalesforceDirectIntegration - Contact',
          contact,
        },
        funcName,
      );
      ...
      request.log.info(
        {
          label:
            'contactActivityController.logActivityToSalesforceDirectIntegration - User Id:',
          user_id,
        },
        funcName,
      );
      ...
      request.log.info(
        {
          label:
            'contactActivityController.logActivityToSalesforceDirectIntegration - Agency User:',
          agency_user,
        },
        funcName,
      );
      ...
      switch (activity_type) {
        case constant.CONTACT.ACTIVITY.TYPE.BUYER_LINK_OPENED: {
          const payload = {
            Title: `Note by Pave`,
            // Timestamp: new Date().getTime(),
            ParentId: source_contact_id,
            Body: `Pave - Link Opened | Link - ${config.webUrl}/preview?permalink=${contact.permalink}`,
          };

          await h.salesforce.sendSalesforceContactNote(
            request,
            agency_id,
            payload,
          );
        ...
        case constant.CONTACT.ACTIVITY.TYPE.COMMENT_POSTED: {
          ...
          if (activityMetaJson.shortlisted_project_id) {
            ...
            payload = {
              ...
              // Timestamp: new Date().getTime(),
              ParentId: source_contact_id,
              Body: `Pave - Comments | ${message} | project - ${name}`,
            };
          } else {
            const { shortlisted_property_id, shortlisted_property_comment_id } =
              activityMetaJson;
            const shortListedProperty =
              await shortListedPropertyController.findOne(
                { shortlisted_property_id },
            ...
            payload = {
              ...
              // Timestamp: new Date().getTime(),
              ParentId: source_contact_id,
              Body: `Pave - Comments | ${message} | property - ${propertyDetails}`,
            };
          }
        ...
        case constant.CONTACT.ACTIVITY.TYPE.PROPERTY_RATED: {
          ...
          if (activityMetaJson.shortlisted_project_id) {
            ...
            payload = {
              ...
              // Timestamp: new Date().getTime(),
              ParentId: source_contact_id,
              Body: `Pave - Ratings | rated ${project_rating} stars on | project - ${name}`,
            };
          } else {
            const { shortlisted_property_id, property_rating } =
              activityMetaJson;
            ...
            payload = {
              ...
              // Timestamp: new Date().getTime(),
              ParentId: source_contact_id,
              Body: `Pave - Ratings | rated ${property_rating} stars on | property - ${propertyDetails}`,
            };
          }
        ...
        case constant.CONTACT.ACTIVITY.TYPE.PROPOSAL_SENT: {
          const payload = {
            ...
            // Timestamp: new Date().getTime(),
            ParentId: source_contact_id,
            Body: `Pave - Proposal Sent | Link - ${config.webUrl}/preview?permalink=${contact.permalink}`,
          };
        ...
        case constant.CONTACT.ACTIVITY.TYPE.UPDATED_PROPOSAL_SENT: {
          const payload = {
            ...
            // Timestamp: new Date().getTime(),
            ParentId: source_contact_id,
            Body: `Pave - Proposal Updated | Link - ${config.webUrl}/preview?permalink=${contact.permalink}`,
          };
  ...
  /**
   * Log contact activity to HubSpot Direct
   * @param {{
   *  contact_fk: string,
   *  activity_type: string,
   *  activity_meta: string
   ...
   * @returns {Promise<void>}
   */
  contactActivityController.logActivityToHubSpotDirect = async (
    record,
    { transaction } = {},
    request = {},
  ) => {
    ...
    const funcName = 'contactActivityController.logActivityToHubSpotDirect';
    try {
      h.validation.requiredParams(funcName, { record });
      const { contact_fk, activity_type, activity_meta } = record;
      h.validation.validateConstantValue(
      ...
      if (h.isEmpty(agency_user_tray)) {
        ...
        switch (activity_type) {
          case constant.CONTACT.ACTIVITY.TYPE.BUYER_LINK_OPENED: {
            ...
            await h.hubspot.sendHubspotContactNote(request, agency_id, payload);
            break;
          }
          case constant.CONTACT.ACTIVITY.TYPE.COMMENT_POSTED: {
            let payload;
  ...
  contactActivityController.calculateLeadScore = async (
    ...
    switch (activity_type) {
      ...
      case constantActivityType.CAROUSEL_PLAY_VIDEO:
      case constantActivityType.CAROUSEL_PLAY_3D:
      ...
      case constantActivityType.TAG_CLICKED_FACTSHEET:
      case constantActivityType.TAG_CLICKED_PROJECT:
      case constantActivityType.TAG_CLICKED_RENDER_3D:
      ...
      case constantActivityType.PROJECT_LEVEL_TAG_CLICKED_FACTSHEET:
      case constantActivityType.PROJECT_LEVEL_TAG_CLICKED_PROJECT:
      case constantActivityType.PROJECT_LEVEL_TAG_CLICKED_RENDER_3D:
        leadScore += 5;
        break;
  ...
  /**
   * Get score per ativity type
   * @param {string} activity_type
   * @param {object} activity_meta
   * @param {{ transaction?:object }} [options]
   * @returns {Promise<void>}
   */
  contactActivityController.getActivityScore = async (
    activity_type,
    activity_meta,
    contact_fk,
  ) => {
    const funcName = 'contactActivityController.getActivityScore';
    h.validation.requiredParams(funcName, { activity_type });
    let contactActivity;
    let parsedActivityMeta;
    const constantActivityType = constant.CONTACT.ACTIVITY.TYPE;
    switch (activity_type) {
        ...
        try {
          ...
          if (contactActivity.length > 1) {
            try {
              ...
              return previousPropertyRating * 10;
            } catch (err) {
              Sentry.captureException(err);
              console.log(
                `${funcName}: Previous rating exists but has no meta data, lead score not changed`,
          ...
          // + current rating score
          return parsedActivityMeta.property_rating * 10;
        } catch (err) {
          Sentry.captureException(err);
          console.log(
            `${funcName}: Activity_type ${activity_type} should have meta data, lead score not changed`,
        ...
        try {
          ...
          if (contactActivity.length > 1) {
            try {
              ...
              return previousProjectRating * 10;
            } catch (err) {
              Sentry.captureException(err);
              console.log(
                `${funcName}: Previous rating exists but has no meta data, lead score not changed`,
          ...
          // + current rating score
          return parsedActivityMeta.project_rating * 10;
        } catch (err) {
          Sentry.captureException(err);
          console.log(
            `${funcName}: Activity_type ${activity_type} should have meta data, lead score not changed`,
      ...
      case constant.CONTACT.ACTIVITY.TYPE.PROJECT_CAROUSEL_LEFT_BUTTON_CLICKED:
      case constant.CONTACT.ACTIVITY.TYPE.PROJECT_CAROUSEL_RIGHT_BUTTON_CLICKED:
      case constant.CONTACT.ACTIVITY.TYPE.READ_WHATSAPP:
        return 2;
      case constantActivityType.CLICKED_ON_WA_BTN_1:
        return 50;
      case constantActivityType.PROPERTY_BOOKMARKED:
        return 30;
      case constantActivityType.PROPERTY_UNBOOKMARKED:
  ...
  /**
   * Get device type activity
   * @param {object} viewed_on_device
   * @param {{ transaction?:object }} [options]
   * @returns {Promise<void>}
   */
  contactActivityController.getDeviceType = async (viewed_on_device) => {
    if (/Windows/i.test(viewed_on_device)) {
      return 'Windows';
    } else if (/Mac/i.test(viewed_on_device)) {
      return 'Mac';
    } else if (/Linux/i.test(viewed_on_device)) {
      return 'Linux';
    } else if (/iPhone/i.test(viewed_on_device)) {
      return 'iPhone';
    } else if (/Android/i.test(viewed_on_device)) {
      return 'Android';
    } else {
      return 'Unknown';
    }
  ...
  contactActivityController.handle3MinuteActivityEmail = async (
    ...
    // Get Updated lead score and past 24 hr score
    const contact = await contactController.findOne({ contact_id });

    engagement_score =
      contact.lead_score + '/' + contact.last_24_hour_lead_score;
    ...
    const shortlisted_properties = await shortListedPropertyController.findAll(
      { contact_fk: contact_id, is_deleted: 0 },
      {
        include: [
          {
            model: models.project_property,
    ...
    for (let i = 0; i < contactActivities.length; i++) {
      ...
      // set total page time via `on_screen_duration`
      if (!h.isEmpty(meta.on_screen_duration)) {
        totalPageTime += meta.on_screen_duration;
      }
      ...
      if (h.general.isProjectMediaTag(activity.dataValues.activity_type)) {
        ...
        if (
          ...
          activity_summary.data[media_tag.project_fk].properties[
            meta.shortlisted_property_id
          ].overall_time += meta.on_screen_duration;

          // Update time for tag
          activity_summary.data[media_tag.project_fk].properties[
            meta.shortlisted_property_id
          ].time_spent[tag] += meta.on_screen_duration;
        }
      } else if (
        activity.dataValues.activity_type ===
        ...
        activity.dataValues.activity_type ===
        constant.CONTACT.ACTIVITY.TYPE.CAROUSEL_PLAY_VIDEO
      ) {
        send_email = true;
        let url = '';
        if (h.notEmpty(meta.video_url)) {
          url = meta.video_url;
        }
        const media_tag = await projectMediaController.findOne(
          { URL: url },
        ...
        if (
          h.notEmpty(meta.agency_fk) &&
          (previousContactValues.permalink_template === 'colliers' ||
            previousContactValues.permalink_template === 'ogps' ||
            previousContactValues.permalink_template === 'raeon')
        ) {
          activity_summary.data[meta.project_fk].time_spent.video +=
            meta.on_screen_duration;
        } else {
          if (h.notEmpty(media_tag)) {
            if (h.notEmpty(meta.shortlisted_project_id)) {
              activity_summary.data[media_tag.project_fk].time_spent.video +=
                meta.on_screen_duration;
            }

            if (
              h.notEmpty(meta.shortlisted_property_id) &&
              activity_summary.data[media_tag.project_fk].properties[
                meta.shortlisted_property_id
              ]
            ) {
              activity_summary.data[media_tag.project_fk].properties[
                meta.shortlisted_property_id
              ].video_duration += meta.on_screen_duration;

              activity_summary.data[media_tag.project_fk].properties[
                meta.shortlisted_property_id
              ].time_spent.video += meta.on_screen_duration;
            }
          }
        }
      } else if (
        activity.dataValues.activity_type ===
        constant.CONTACT.ACTIVITY.TYPE.CAROUSEL_PLAY_3D
      ) {
        send_email = true;
        let url = '';
        if (h.notEmpty(meta.iframe_url)) {
          url = meta.iframe_url;
        }
        const media_tag = await projectMediaController.findOne(
          { URL: url },
        ...
        if (
          ...
          activity_summary.data[meta.project_fk].time_spent.render_3d +=
            meta.on_screen_duration;
        } else {
          if (h.notEmpty(media_tag)) {
            if (h.notEmpty(meta.shortlisted_project_id)) {
              activity_summary.data[
                media_tag.project_fk
              ].time_spent.render_3d += meta.on_screen_duration;
            }
    ...
    if (previousContactValues.permalink_template !== 'pave') {
      let projectName = '';
      ...
      console.log('💥💥💥💥💥💥💥 START 💥💥💥💥💥💥💥');
      console.log(previousContactValues.permalink_template);
      console.log('💥💥💥💥💥💥💥 END 💥💥💥💥💥💥💥');

      subject_message = h.getMessageByCode(
        'template-3-minute-buyer-activity-summary-email-subject-1647832776541-landing',
        {
          CONTACT_NAME:
      ...
      body_message = h.getMessageByCode(
        'template-3-minute-buyer-activity-summary-email-front-body-1647832776541-landing',
        {
          ...
          ACTIVITY_DEVICE: h.notEmpty(activity_summary.overall.device)
            ? activity_summary.overall.device
            : '-',
          ENGAGEMENT_SCORE: h.notEmpty(engagement_score)
            ? engagement_score
            : '-',
          PAGE_TIME_SPENT: h.general.prettifySeconds(totalPageTime),
        },
      );
    } else {
      subject_message = h.getMessageByCode(
    ...
    if (
      ![
        ...
        'ogps_burmingham',
        'ogps_park_avenue',
        'ogps_park_avenue_hk_eng',
        'ogps_park_avenue_hk_chn',
        'ogps_expat',
        'ogps_manchester',
        'ogps_mqdc',
        'ogps_melbourne_townhouse',
        'ogps_wembley',
        'ogps_hendon',
        'ogps_hendon_my',
        'ogps_barratt_personal',
        'ogps_barratt_invited',
        'ogps_barratt_my',
        'ogps_park_quarter',
        'ogps_pen_gardens',
        'ogps_vision_point',
        'ogps_perth',
        'ogps_eastman_village',
        'ogps_eastman_village_my',
        'ogps_trafford_gardens',
        'hybrid_hk',
        'hybrid_gym_group',
        'h_kore',
        'j_elliot',
        ...
        'ogps_floret',
        'breathe_pilates',
        'breathe_pilates_classes_prices',
        'breathe_pilates_prospects',
        'breathe_pilates_re_engagement',
      ].includes(previousContactValues.permalink_template)
    ) {
      for (let i = 0; i < Object.keys(activity_summary.data).length; i++) {
        const project =
          activity_summary.data[Object.keys(activity_summary.data)[i]];
        ...
        for (let j = 0; j < Object.keys(properties).length; j++) {
          ...
          const overallTime = h.general.prettifySeconds(
            property.overall_time + property.video_duration,
          );
          const videoPlayTime = h.general.prettifySeconds(
  ...
  contactActivityController.notifyActivityToAgent = async (
    ...
    const canCreateCron =
      await emailNotificationSettingController.ifCanSendEmail(
        previousContactValues.agency_user_fk,
        'engagement_summary',
      );

    const canNotify = await emailNotificationSettingController.ifCanSendEmail(
      previousContactValues.agency_user_fk,
      'proposal_rating',
    );
    ...
    switch (activity_type) {
      case constant.CONTACT.ACTIVITY.TYPE.BUYER_LINK_OPENED:
        ...
        contactActivityOpened = await contactActivityController.findOne({
          contact_activity_id,
        });
        ...
        if (h.isEmpty(activity_summary.overall.device)) {
          activity_summary.overall.device =
            h.contactActivity.prettifyViewOnDeviceString(
              contactActivityOpened.dataValues.viewed_on_device,
            );
        }
        ...
        if (h.isEmpty(activity_summary.overall.location)) {
          const geo = await geoip.lookup(
            contactActivityOpened.dataValues.activity_ip,
          );
          if (h.notEmpty(geo)) {
            activity_summary.overall.location =
              h.general.formateLocationStr(geo);
          }
        }
        ...
        if (
          ...
          engagement_score = previousContactValues.last_24_hour_lead_score;
          ...
          engagement_score =
            previousContactValues.lead_score +
            '/' +
            previousContactValues.last_24_hour_lead_score;
      ...
      case constant.CONTACT.ACTIVITY.TYPE.PROJECT_RATED:
        {
          ...
          const shortlistedProject = await shortListedProjectController.findOne(
            {
              shortlisted_project_id: parsedActivityMeta.shortlisted_project_id,
            },
          );
