...
module.exports.makeAuthController = (models) => {
  ...
  authController.registerUserByGoogle = async (
    ...
    // create user role if registered using the google signup button in login
    if (h.cmpBool(from_login, true)) {
    ...
    if (h.cmpBool(from_login, false)) {
      // Login user using Google signin payload
      const { access_token: token } = await authController.loginUserByGoogle(
        social_payload,
        { transaction },
      );
      access_token = token;
  ...
  authController.loginUserByEmail = async (
    ...
    const verification = await userController.verifyUserPassword(
      email,
      password,
      { transaction },
    );

    if (h.cmpBool(verification?.validated, false)) {
      return {
        user_id: null,
        access_token: null,
    ...
    return { user_id: user.user_id, access_token, error: null };
  };
  ...
  authController.loginUserByGoogle = async (
    ...
    let user = await userController.findOne({ email }, { transaction });
    if (h.isEmpty(user)) {
      await authController.handleRegisterUserByGooleOnLogin(social_payload);
    ...
    if (h.isEmpty(user_social_auth))
      throw new Error(
        `${funcName}: email '${email}' did not have an account with Chaaat via ${constant.USER.AUTH_TYPE.GOOGLE}`,
      );
  ...
  authController.loginUserByFacebook = async (
    ...
    if (h.isEmpty(user_social_auth))
      throw new Error(
        `${funcName}: email '${email}' did not have an account with Chaaat via ${constant.USER.AUTH_TYPE.FACEBOOK}`,
      );
    // Update user's profile picture with the latest from their Google profile
    // await userController.update(user.user_id, { profile_picture_url }, { transaction });
    const { access_token } = await loginUser(user.user_id, { transaction });
  ...
  authController.hasAccessToStaffPortal = async (user_id) => {
    ...
    const userRoleRecord = await userRoleController.findOne({
      [Op.or]: [
        ...
        { user_fk: user_id, user_role: constant.USER.ROLE.SUPER_ADMIN },
      ],
    });
   ...
   * @returns {Promise<boolean>}
   */
  authController.hasAccessToSuperAdminPortal = async (user_id) => {
    const funcName = 'authController.hasAccessToSuperAdminPortal';
    h.validation.requiredParams(funcName, { user_id });
    const userRoleRecord = await userRoleController.findOne({
      user_fk: user_id,
   ...
   * @returns {Promise<void>}
   */
  authController.forgotPassword = async (email) => {
   ...
   * @returns {Promise<{record: (Object|Array), created: boolean}>}
   */
  authController.registerClientSupportEmail = async (
    first_name,
    last_name,
    email,
    password,
    ...
    if (!created) {
      ...
      if (send_email) {
        ...
        await userEmailVerificationController.sendSupportEmailVerification(
          record.user_id,
          agency_name,
          password,
          { transaction },
  ...
  /**
   * Description
   * Function to handle registration when login with google is done and not user record yet
   * @async
   * @constant
   * @name handleRegisterUserByGooleOnLogin
   * @param {object} social_payload google login payload data
   */
  authController.handleRegisterUserByGooleOnLogin = async (social_payload) => {
    const transaction = await models.sequelize.transaction();
    try {
      await authController.registerUserByGoogle(
        social_payload.profileObj.given_name,
        social_payload.profileObj.family_name,
        social_payload.profileObj.email,
        social_payload,
        { buyer_type: null, invitee: null, transaction },
        true,
      );
      await transaction.commit();
    } catch (err) {
