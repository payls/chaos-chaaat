...
module.exports.makeController = (models) => {
  const { automation_rule_form: model } = models;
  const ctr = {};

  ctr.create = async (record, { transaction } = {}) => {
    const funcName = 'ctr.create';
    h.validation.requiredParams(funcName, { record });
    const { automation_rule_fk, form_fk } = record;
    const automation_rule_form_id = h.general.generateId();
    await model.create(
      {
    ...
    return automation_rule_form_id;
  };
  ...
  ctr.update = async (
    ...
    await model.update(
      ...
      { where: { automation_rule_form_id }, transaction },
    );
    return automation_rule_form_id;
  };
