...
module.exports.makeUserEmailVerificationController = (models) => {
  ...
  /**
   * Send email already exists email to user by user id
   * @param {string} user_id
   * @param {{ transaction?:object }} [options]
   * @returns {Promise<void>}
   */
  userEmailVerificationController.sendUserExistsEmail = async (
    user_id,
    { transaction } = {},
  ) => {
    const funcName = 'userEmailVerificationController.sendUserExistsEmail';
    h.validation.requiredParams(funcName, { user_id });
    const user = await userController.findOne({ user_id }, { transaction });

    const login_url = `${config.webAdminUrl}/login`;
    await h.email.sendEmail(
      `Chaaat Team <registrations@${config?.email?.domain || 'chaaat.io'}>`,
      user.email,
      null,
      h.getMessageByCode('template-emailAlreadyExists-subject-1634979920823'),
      h.getMessageByCode('template-emailAlreadyExists-body-1634979963140', {
        FIRST_NAME: user.first_name,
        LOGIN_URL: login_url,
      }),
    );
  ...
  /**
   * Send email verification email to support
   * @param {string} user_id
   * @param {{ transaction?:object }} [options]
   * @returns {Promise<void>}
   */
  userEmailVerificationController.sendSupportEmailVerification = async (
    user_id,
    agency_name,
    password,
    { transaction } = {},
