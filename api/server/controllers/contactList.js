...
module.exports.makeController = (models) => {
  ...
  contactList.findAll = async (
    where,
    { order, include, transaction, attributes, offset, limit } = {},
  ) => {
  ...
  /**
   * Find all records and also return total count regardless of offset and limit
   * @param {object} where
   * @param {{ transaction?:object }} [options]
   * @returns {Promise<Object>}
   */
  contactList.findAndCountAll = async (
    where,
    { order, include, transaction, attributes, offset, limit } = {},
  ) => {
    const funcName = 'contactList.findAndCountAll';
    h.validation.requiredParams(funcName, { where });
    h.validation.isObjectOrArray(funcName, { where });
    const record = await contactListModel.findAndCountAll({
      where: { ...where },
      offset,
      limit,
      order,
