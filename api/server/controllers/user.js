...
module.exports.makeUserController = (models) => {
  ...
  userController.updatePassword = async (
    ...
    const user = await userModel.findOne({ where: { user_id }, transaction });
  ...
  userController.verifyUserPassword = async (
    ...
    if (!h.cmpStr(hashed_password, user.password)) {
      return { validated: false, message: '2-auth-1608510138480' };
    }
   ...
   */
  userController.findOne = async (where, { transaction } = {}) => {
  ...
  userController.validateIfWhatsAppMobile = async (
    { user_id, api_credentials },
    { transaction } = {},
  ) => {
    const funcName = 'userController.validateIfWhatsAppMobile';
    h.validation.requiredParams(funcName, { user_id });
    const { mobile_number, is_whatsapp } = await userController.findOne(
      { user_id },
      { transaction },
    );
    if (h.cmpBool(is_whatsapp, true))
      return {
      ...
      if (h.isEmpty(mobile_number)) {
        ...
        const agentMobile = mobile_number.replace(/[^0-9,;\-.!? ]/g, '');
        const environment = h.cmpStr(process.env.NODE_ENV, 'production')
          ? 'whatsappofficial'
          : 'whatsappsandbox';
        const connectionData = JSON.stringify({
          uri: `${environment}://${agentMobile}@whatsapp.com`,
          name: `${agentMobile}`,
        });
