...
async function updateContact({
  ...
  await models.contact_source.update(
    { source_type: 'HUBSPOT' },
...
async function load(result) {
  ...
  if (
    ...
    contactOwnerFullName === 'Raeon Info'
  ) {
    user = await models.user.findOne({
      where: {
        email: 'info@rae-on.com',
      },
      include: [
        {
  ...
  //   contactOwnerFullName === 'Raeon Info'
  // ) {
  //   wrongUser = await models.user.findOne({
  //     where: {
  //       email: 'info@rae-on.com',
  //     },
  //     include: [
  ...
  const mobile_number = mobile; // mobileNumberChecker(mobile);
  ...
  // if (
  //   wrongUser.first_name + ' ' + wrongUser.last_name !==
  //   user.first_name + ' ' + user.last_name
  // ) {
  //   console.log(
  //     `${contactHsId},${
  //       contactSource.contact.contact_id
  //     },${first_name},${last_name},${email},${
  //       contactSource.contact.mobile_number
...
async function loadAll(results) {
  console.log(
    'HB_ID,contact_id,first_name,last_name,email,mobile,contact_in_csv,wrong_contact_owner,right_contact_owner',
  );
...
csv()
  .fromFile(path.resolve('.', 'raeon_export_feb16.csv'))
  .then((results) => {
