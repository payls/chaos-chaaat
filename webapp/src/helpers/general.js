...
export function customizedAlert(
  type = 'success',
  options = { message: '', autoCloseInSecs: 5, cl: '' },
) {
  type = type || 'success';
  const toastOptions = {
    className: `toast-${type}-${options.cl}`,
    position: 'top-center',
    autoClose: options.autoCloseInSecs * 1000,
  };
...
/**
 * Obtain agency name from permalink url
 * @param {string} url
 * @returns {string}
 */
export function getAgencyName(url) {
  let { permalink: inPermalink } = url;
  if (notEmpty(inPermalink)) {
    inPermalink = inPermalink.split('-')[0];
  }
  return inPermalink;
}
