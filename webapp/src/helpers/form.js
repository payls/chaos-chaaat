...
export function CustomForm(props) {
  ...
  const onSubmitWithCaptcha = async (e) => {
    e.preventDefault();
  ...
  return (
    <Form
      className={className}
      inline={inline}
      onSubmit={reCaptcha ? onSubmitWithCaptcha : onSubmit}
      style={style}
    >
      {reCaptcha && (
        <ReCAPTCHA
          ref={recaptchaRef}
          size="invisible"
          sitekey="6LdFp18pAAAAAOl3W-glomoOcUU-MOB2ZJIqbUrk"
...
export function GenericForm(props) {
  ...
  return (
    <h.form.CustomForm onSubmit={preHandleSubmit} reCaptcha={reCaptcha}>
          ...
          Object.keys(formFields).map((fieldKey, i) => {
            ...
            switch (fieldType) {
              ...
              case h.form.FIELD_TYPE.TEXTAREA:
                return visible ? (
                  <div className={class_name} key={elKey}>
                    <h.form.CustomInput
                      label={label}
                      data={fields[fieldKey]}
                      name={fieldKey}
              ...
              case h.form.FIELD_TYPE.PHONENUMBER:
                return visible ? (
                  <div className={class_name} key={i}>
                    <div className="prependedCountry">
                      <Select
                        defaultValue={{
                          label: `🇸🇬 SG +65`,
                          value: {
