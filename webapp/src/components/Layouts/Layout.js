...
export function Header({
  ...
  url = null,
  favicon = 'https://cdn.yourpave.com/assets/favicon-32x32-kH8gKq9n.png',
}) {
  if (h.notEmpty(title)) {
    if (titlePaveEnding) {
      title = title + ' | Pave';
    ...
    title = 'Pave - Share & Track Project Information with Prospects';
  }
  const router = useRouter();
  const current_url = `${config.webUrl}${url ? url : router.pathname}`;
  return (
    <div>
      <Head>
        <title>{title}</title>
