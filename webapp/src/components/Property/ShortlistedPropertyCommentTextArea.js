...
export default function ShortlistedPropertyCommentTextArea(props) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    let apiResComment = await api.shortlistedPropertyComment.create(
      {
        shortlisted_property_id,
        contact_id,
        ...
        parent_comment_fk: h.notEmpty(replyToComment)
          ? replyToComment.shortlisted_property_comment_id
          : undefined,
        project,
      },
      false,
    ...
    if (h.cmpStr(apiResComment.status, 'ok')) {
      ...
      createdCommentId = apiResComment.data.shortlisted_property_comment_id;
    ...
    let apiResContactActivity = await api.contactActivity.create(
      {
        ...
        activity_meta: JSON.stringify({
          shortlisted_property_id,
          shortlisted_property_comment_id: createdCommentId,
        }),
      },
      false,
    );
