...
export default function TaskListingLaneItem({ setLoading, task }) {
  const { task_id, subject, message_latest, status_updated_date_time_ago } =
    task;
  ...
  return (
    <Card className="mx-auto mb-2" style={{ maxWidth: '30rem' }}>
      <Card.Body>
        ...
        {/*<Card.Text className="float-right text-muted"><small>{status_updated_date_time_ago}</small></Card.Text>*/}
      </Card.Body>
    </Card>
  );
}
