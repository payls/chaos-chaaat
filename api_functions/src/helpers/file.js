 ...
 * @param {{file_name:string}} options
 * @returns {string}
 */
fileHelper.getFilePath = (uploadType, options) => {
  ...
  switch (uploadType) {
    case constant.UPLOAD.TYPE.USER_PROFILE_IMAGE:
      filePath = `user/profile/${hashedFileNameOnly}.${fileExtOnly}`;
      break;
    case constant.UPLOAD.TYPE.PROJECT_PROPERTY_MEDIA_IMAGE:
      filePath = `project/property/media/image/${hashedFileNameOnly}.${fileExtOnly}`;
      break;
    case constant.UPLOAD.TYPE.PROJECT_PROPERTY_MEDIA_VIDEO:
      filePath = `project/property/media/video/${hashedFileNameOnly}.${fileExtOnly}`;
      break;
    case constant.UPLOAD.TYPE.PROJECT_PROPERTY_MEDIA_YOUTUBE:
      filePath = `project/property/media/youtube/${hashedFileNameOnly}.${fileExtOnly}`;
      break;
    case constant.UPLOAD.TYPE.SHORTLISTED_PROPERTY_COMMENT_ATTACHMENT:
      filePath = `shortlisted_property/comment/attachment/${hashedFileNameOnly}.${fileExtOnly}`;
      break;
    case constant.UPLOAD.TYPE.PROJECT_PROPERTY_HEADER_INFO_COVER_PICTURE:
      filePath = `project/cover/${hashedFileNameOnly}.${fileExtOnly}`;
      break;
    case constant.UPLOAD.TYPE.PROJECT_BROCHURE:
      filePath = `project/brochure/${hashedFileNameOnly}.${fileExtOnly}`;
      break;
    case constant.UPLOAD.TYPE.PROJECT_MEDIA_IMAGE:
      filePath = `project/media/image/${hashedFileNameOnly}.${fileExtOnly}`;
      break;
    case constant.UPLOAD.TYPE.PROJECT_MEDIA_VIDEO:
      filePath = `project/media/video/${hashedFileNameOnly}.${fileExtOnly}`;
      break;
    case constant.UPLOAD.TYPE.PROJECT_TEAM_BEHIND_LOGO:
      filePath = `project/team_behind/${hashedFileNameOnly}.${fileExtOnly}`;
      break;
    case constant.UPLOAD.TYPE.REPORT:
      filePath = `reports/${hashedFileNameOnly}.${fileExtOnly}`;
      break;
  }
  return filePath;
};
