...
emailHelper.sendEmail = async (
  ...
  try {
    ...
    const html = await fs.readFile(
      `src/locales/${options.language_code}/templates/email/${options.template}.html`,
      'utf8',
    );
...
emailHelper.sendHtmlEmail = async (from, to, subject, html) => {
  const funcName = 'emailHelper.sendHtmlEmail';
  const emailData = { from, to, subject, html: '' };
  try {
    if (!from) throw new Error(`${funcName}: missing param from`);
    if (!to) throw new Error(`${funcName}: missing param to`);
    if (!subject) throw new Error(`${funcName}: missing param subject`);
    if (!html) throw new Error(`${funcName}: missing param html`);
    // Initialize mailgun
    ...
    emailData.html = html;
    if (h.test.isTest()) {
      console.info(`${funcName}: simulating sending of email`, emailData);
    } else {
      // Send email
    ...
    if (err)
      console.info(`${funcName}: failed to send email`, { emailData, err });
  }
};
/**
 * Generic send email function
