...
const sendNotification = async (event = {}) => {
  const functionName = 'SEND_FINISHED_CAMPIGN_NOTIFICATION';
  try {
    console.info('START SEND_FINISHED_CAMPIGN_NOTIFICATION', event);
    ...
    await Promise.mapSeries(runningSchedules, async (schedule) => {
      ...
      const processedMainTrackerCount =
        await models.whatsapp_message_tracker.count({
          where: {
      ...
      const processedSubTrackerCount =
        await models.whatsapp_message_tracker.count({
          where: {
            tracker_ref_name: schedule.tracker_ref_name,
            tracker_type: 'sub',
      ...
      if (
        ...
        const data = JSON.stringify({
          text: `${env_details}<!here> ${schedule.campaign_name} sending with ${schedule.recipient_count} recipient(s) for ${agency.agency_name} is finished. Thank you.`,
        });
    ...
    console.info('END SEND_FINISHED_CAMPIGN_NOTIFICATION', event);
