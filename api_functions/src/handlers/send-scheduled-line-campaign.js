...
const sendCampaign = async (event = {}) => {
  const functionName = 'SEND_SCHEDULED_LINE_CAMPAIGN';
  try {
    console.info('START SEND_SCHEDULED_LINE_CAMPAIGN', event);
    ...
    await Promise.mapSeries(schedules, async (schedule) => {
      ...
      if (general.cmpStr(timeZoneTime, scheduledTime)) {
        console.info('matched', timeZoneTime, scheduledTime);
        ...
        try {
          ...
          if (general.cmpStr(event_type, 'reminder')) {
            const data = JSON.stringify({
              text: `<!here> This is a reminder that a Line campaign for ${schedule.campaign_name} with ${schedule.recipient_count} recipient(s) for ${agency.agency_name} will automatically start in ${payload}. Thank you.`,
            });
            ...
            const data = JSON.stringify({
              text: `<!here> Automated Line campaign for ${schedule.campaign_name} with ${schedule.recipient_count} recipient(s) for ${agency.agency_name} will start now. Thank you.`,
            });
            ...
            triggered = await rabbitmq.pubBulkProcessLineCampaign(payload);
          }
          if (triggered) {
    ...
    console.info('END SEND_SCHEDULED_LINE_CAMPAIGN', event);
