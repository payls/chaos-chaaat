          ...
          <div className="col-12">
            {!h.cmpStr(taskStatus, constant.TASK.STATUS.COMPLETED) && (
              <h.form.CustomButton
                ...
                size="sm"
                onClick={async () => {
                  const apiRes = await api.task.complete({ task_id: taskId });
                  if (h.cmpStr(apiRes.status, 'ok')) {
                    closeDialog();
                  }
                }}
            ...
            {h.cmpStr(taskStatus, constant.TASK.STATUS.COMPLETED) && (
              <h.form.CustomButton
                ...
                size="sm"
                onClick={async () => {
                  const apiRes = await api.task.restore({ task_id: taskId });
                  if (h.cmpStr(apiRes.status, 'ok')) {
                    closeDialog();
                  }
                }}
          ...
          h.notEmpty(task.task_messages) && (
            ...
            >
              {task.task_messages.map((task_message) => {
                ...
                let { full_name, profile_picture_url } = task_message.user;
                return (
                  <ListGroup.Item
                    variant={
                      h.cmpStr(type, constant.TASK.MESSAGE.TYPE.STAFF_TO_CLIENT)
                        ? 'primary'
                        : ''
                    }
                  >
