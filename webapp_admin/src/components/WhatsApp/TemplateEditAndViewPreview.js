...
export default React.memo(
  ({
    ...
    const isVideo = /\.(mp4|3gp)$/i.test(image);
          ...
          <div className="iphone-frame-body">
            <div className="template-preview-wrapper">
              <div>
                {header !== 'none' && (
                  <div className="template-image-banner">
                    ...
                    {h.notEmpty(image) && h.cmpStr(header, 'image') && (
                      <img src={!isVideo ? image : undefined} width={'100%'} />
                    )}
                    {h.notEmpty(image) && h.cmpStr(header, 'video') && (
                      <video style={{ width: '100%', marginTop: '30px' }} controls src={isVideo ? image : undefined}></video>
                    )}
                  </div>
                )}
                ...
                {h.notEmpty(data.template_body) && <small>{getTime()}</small>}
                {h.notEmpty(cta) &&
                  cta
