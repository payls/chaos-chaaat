...
export default function PropertyModal(props) {
  ...
  const hasMedia = useCallback(
    (type) =>
      unit.medias
        .filter((f) => f.media_tag)
        .filter((f) => f.media_tag.includes(type)).length > 0,
    [unit.medias],
  );
  ...
  const handleSetImageTriggerUnits = (v) => {
    ...
    if (v.media_order !== null && v.media_order !== '') {
      ...
      reorderMediasArray = unit.medias.sort(
        (a, b) => a.media_display_order - b.media_display_order,
      );
    }
      ...
      reorderMediasArray
        ...
        .filter(
          (f) =>
            ...
            (v?.media_setting_factsheet && f.media_tag.includes('factsheet')) ||
                        ...
                        <div className="slideeeee">
                          {filteredUnits.filter((f) => !f.isHidden).length >
                            ...
                            <CommonCarousel
                              ...
                              key={
                                shortlisted_property_id + '-carousel-property'
                              }
                              items={filteredUnits
                                .filter((f) => !f.isHidden)
                                .map((media, index) => {
