...
/**
 * Get Shortlisted template table list
 * @param {object} [data]
 * @param {boolean} [showMessage=true]
 * @returns {Promise<{data: {}, status: string}>}
 */
export async function getShortlistedTemplateTableList(data, showMessage) {
  const response = await Axios({
    url: `${config.apiUrl}/v2/contact/${data.proposal_template_id}/shortlist-template-table`,
    method: 'get',
    headers: {
      Accept: 'application/json',
      'Content-Type': 'application/json',
