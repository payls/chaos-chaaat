...
export default React.memo(({ agency_id, rule, handleCloseModal }) => {
  ...
  useEffect(() => {
    (async () => {
      const response = await api.automation.getRecipients(
        {
          agency_id,
  ...
  }, [rule, dateFilter, selectedMessageStatus, debouncedQuery]);
        ...
        >
          ...
          <div>
            <DateFilter
              ...
              def={{ value: 'mtd', label: 'Month to Date' }}
            />
          </div>
            ...
            <table className="contact-summary-table">
              <thead>
                <tr>
                  ...
                  <th style={{ width: '80px', textAlign: 'center' }}>Failed</th>
