    ...
    (async () => {
      if (
        ...
        if (h.cmpStr(draftRes.status, 'ok')) {
          ...
          getTemplates(agency.agency_id, { value: line });
    ...
    h.general.prompt(
      ...
      async (submitCampaign) => {
        if (h.cmpBool(submitCampaign, true)) {
          ...
          const apiRes = await api.campaignSchedule.draftLineCampaignUpdate(
            campaign_draft_id,
            formData,
            false,
          );
    ...
    h.general.prompt(
      ...
      async (submitCampaign) => {
        if (h.cmpBool(submitCampaign, true)) {
          ...
          const apiRes =
            await api.campaignSchedule.draftLineCampaignUpdateReview(
              campaign_draft_id,
              formData,
              false,
            );
