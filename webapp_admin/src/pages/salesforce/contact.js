...
// Components
import Contacts from '../../components/Salesforce/Contacts';
...
export default function CampaignTemplateList() {
  ...
  const { linkDetails, setStoreLinkDetails, contactsArray, sfObjectStore } =
    useSalesforceStore();
  ...
  async function importContacts() {
    h.general.prompt(
      {
        message: `Are you sure to import selected contacts?`,
      },
      ...
      async (status) => {
        if (status) {
          ...
          for (let i = 0; i < chunkedArray.length; i++) {
            importRes = await api.contact.importSalesforceContacts(
              {
                sf_contact_ids: chunkedArray[i],
              },
              agency.agency_id,
              sfObjectStore.value,
            );
