...
/**
 * Description
 * Consumer for handling webhook payloads coming from contact update action in
 * hubspot
 * This will include receiving of webhook payload data coming from hubspot when
 * a contact has been update
 *
 * @param {object} data holds hubspot webhook payload for update contact action
...
module.exports = async ({ data, models, channel, config, pubChannel, log }) => {
  ...
  const processor_name = 'hs-update-contact';
  ...
  try {
    for (const agency of agencies) {
      await handleContactUpdateWebhookPayloadFromHubSpot(
...
/**
 * Description
 * Function to handle every update webhook payload received by system from
 * hubspot
 * @async
 * @function
 * @name handleContactUpdateWebhookPayloadFromHubSpot
 * @kind function
 ...
 */
async function handleContactUpdateWebhookPayloadFromHubSpot(
  { agency, config, process_id, processor_name, body, models },
  { log },
) {
  try {
    ...
    if (h.cmpBool(refreshTokenResponse.success, false)) {
      ...
      if (h.isEmpty(contact) || h.cmpBool(contact, false)) {
        ...
        const contact_id = await c.hubspot.updateContactInPaveV3(
          {
            contact: {
              properties: {
        ...
        if (h.notEmpty(contact_id)) {
          await h.hubspot.hubspotProcessFormSubmissionForUpdateWebhook(
            {
              contact,
              agency,
              contact_id,
