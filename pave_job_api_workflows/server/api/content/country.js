...
// 		response.data.sort((a, b) => (a.title.rendered > b.title.rendered) ? 1 : ((b.title.rendered > a.title.rendered) ? -1 : 0));
// 		//Filter records that are not visible in production
// 		if (h.cmpStr(config.env, 'production')) {
// 			response.data = response.data.filter((item) => h.cmpInt(item.visible_in_production, 1));
// 		}
// 	}
