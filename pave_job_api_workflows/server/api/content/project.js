...
const projectApi = module.exports;

projectApi.getProjectBySlug = async (data) => {
  ...
  const apiRes = { status: 'ok', data: { project: {} } };
...
function formatProjectData(project) {
  const formattedData = JSON.parse(JSON.stringify(projectSchema));
  ...
  if (project.header_descriptions) {
    const headerDescriptions = project.header_descriptions.split(';');
  ...
  if (project.project_images) {
    project.project_images.forEach((image, index) => {
      const convertAltString = `${project.slug}-slider-${index + 1}`;
      const imageObj = {
        src: image.guid ? image.guid : '',
        alt: convertAltString || '',
      };
