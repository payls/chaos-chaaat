 ...
 * @returns
 */
generalHelper.replaceMessage = (message = '', messageData = {}) => {
  if (!generalHelper.isEmpty(messageData)) {
    for (const key in messageData) {
      message = message.replaceAll(`[${key}]`, messageData[key]);
    }
...
generalHelper.getUIBChannelUserProfile = async ({
  user_profile_id,
  api_token,
  api_secret,
}) => {
  const userData = JSON.stringify({
    uri: `unified://${user_profile_id}`,
  });
  ...
  const userDataConfig = {
    method: 'post',
    url: 'https://apiv2.unificationengine.com/v2/connection/info',
    headers: {
  ...
  const userProfile = await axios(userDataConfig)
  ...
  if (!generalHelper.isEmpty(userProfile.connectors)) {
    return userProfile.connectors[user_profile_id][0];
  } else {
    return null;
  }
};

generalHelper.getLineDirectChannelUserProfile = async ({
  user_profile_id,
  api_token,
  api_secret,
}) => {
  const userDataConfig = {
    method: 'get',
    url: `https://api.line.me/v2/bot/profile/${user_profile_id}`,
    headers: {
  ...
  if (!generalHelper.isEmpty(userProfile)) {
    return userProfile;
  } else {
    return null;
  }
...
generalHelper.getMessengerUserProfile = async ({
  user_profile_id,
  api_token,
}) => {
  const userDataConfig = {
    method: 'get',
    url: `https://graph.facebook.com/v19.0/${user_profile_id}?fields=first_name,last_name&access_token=${api_token}`,
  };
...
generalHelper.unescapeData = (data) => {
  if (typeof data === 'object' && data !== null) {
    ...
    return generalHelper.sanitizeMaliciousAttributes(unescapedVal);
 ...
 */
generalHelper.sanitizeMaliciousAttributes = (message) => {
