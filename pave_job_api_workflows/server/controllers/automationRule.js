...
module.exports.makeController = (models) => {
  ...
  ctr.checkIfRuleExistsAndIsActive = async (filters) => {
    const funcName = 'ctr.checkIfRuleExistsAndIsActive';
    h.validation.requiredParams(funcName, { filters });
    const { agency_id, trigger, agency_whatsapp_config_id } = filters;
    const rule = await ctr.findOne(
      {
        status: 'active',
        rule_trigger_fk: trigger,
      },
      {
        include: [
          {
            ...
            where: {
              ...
              business_account: agency_whatsapp_config_id,
            },
            required: true,
          },
          {
