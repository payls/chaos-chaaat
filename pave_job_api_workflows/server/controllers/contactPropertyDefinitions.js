...
module.exports.makeContactPropertyDefinitionsController = (models) => {
  ...
  /**
   * Bulk update contact_property_definitions record
   * @param {{
   ...
   * @returns {Promise<string>}
   */
  contactPropertyDefinitionsController.bulkUpdate = async (
    where,
    record,
    { transaction } = {},
  ) => {
    const funcName = 'contactPropertyDefinitionsController.bulkUpdate';
    h.validation.requiredParams(funcName, {
      where,
      record,
    });
    ...
    return await contactPropertyDefinitionsModel.update(
      {
        agency_user_fk,
        agency_fk,
        attribute_name,
