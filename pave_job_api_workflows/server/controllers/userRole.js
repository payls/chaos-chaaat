...
module.exports.makeUserRoleController = (models) => {
  ...
  /**
   * find or create method for userRoleModel
   * @param where
   * @param defaults
   * @param transaction
   * @returns {Promise<{created: *, record: (Object|Array)}>}
   */
  userRoleController.findOrCreate = async (
    where,
    defaults,
    { transaction } = {},
  ) => {
    const funcName = 'userRoleController.findOrCreate';
    const { user_fk, user_role, created_by } = defaults;
    ...
    const user_role_id = h.general.generateId();
    const [record, created] = await userRoleModel.findOrCreate({
      where: { ...where },
      defaults: {
        user_role_id,
        user_fk,
