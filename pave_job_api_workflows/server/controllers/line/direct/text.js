...
module.exports.makeController = (models) => {
  ...
  lineCtl.processTextMessage = async (data, log, { transaction } = {}) => {
    try {
      const funcName = 'lineCtl.processTextMessage';
      const {
        agency_id,
      ...
      log.info({
        message: `Contact processing in Line direct text API call took ${contactProcessingDuration} milliseconds`,
      });
      ...
      if (h.cmpBool(is_opt_in_message, true)) {
        const hasOptInMessageRecordFromAgent = await lineChatModel.findOne({
          where: {
            contact_fk: contact_id,
            msg_type: 'frompave',
            msg_origin: 'opt_in',
        ...
        if (h.isEmpty(hasOptInMessageRecordFromAgent)) {
          const opt_in_line_chat_id = h.general.generateId();
          await lineChatModel.create(
            {
              line_chat_id: opt_in_line_chat_id,
              campaign_name,
              agency_fk: agency_id,
              agency_user_fk: agency_user_id,
              contact_fk: contact_id,
              ...
              reply_to_msg_type,
              reply_to_contact_id,
              created_date: opt_in_message_sent_date,
            },
            { transaction },
          );

          const options = { day: 'numeric', month: 'short', year: 'numeric' };
          const date = new Date(opt_in_message_sent_date);
          ...
          await h.appsync.sendGraphQLNotification(api_key, {
            ...
            read: 1,
            replied: 1,
            created_date_raw: new Date(opt_in_message_sent_date),
          ...
          if (h.cmpBool(new_contact, false)) {
            ...
            if (!h.isEmpty(contact_source)) {
              ...
              await h.salesforce.transmitMessage({
                ...
                full_message_body: opt_in_message,
      ...
      log.info({
        message: `Message processing in Line direct text API call took ${messageProcessingDuration} milliseconds`,
      });
      return { success: true, data: contact_id };
    } catch (err) {
      return { success: false, error: 'TEXT PROCESSING ERROR', data: err };
    }
  };
  return lineCtl;
};
