...
module.exports.makeController = (models) => {
  ...
  lineCtl.processVideoMessage = async (data, log, { transaction } = {}) => {
    try {
      const funcName = 'lineCtl.processVideoMessage';
      const {
        agency_id,
      ...
      log.info({
        message: `Contact processing in Line direct video API call took ${contactProcessingDuration} milliseconds`,
      });
      ...
      log.info({
        message: `Message processing in Line direct video API call took ${messageProcessingDuration} milliseconds`,
      });
      return { success: true, data: contact_id };
    } catch (err) {
      return { success: false, error: 'VIDEO PROCESSING ERROR', data: err };
    }
  };
