...
module.exports.makeController = (models) => {
  ...
  lineCtl.processVideoMessage = async (data, log, { transaction } = {}) => {
    ...
    const line_channel = await agencyChannelConfigModel.findOne({
      where: {
        agency_fk: agency_id,
        channel_type: 'line',
    ...
    const api_credentials = Buffer.from(
      `${line_channel?.dataValues?.uib_api_token}:${line_channel?.dataValues?.uib_api_secret}`,
      'utf8',
    ).toString('base64');
    ...
    const mediaURL = await h.line.getMediaURL(
      {
        msg_type: msg_type,
        message_id: content,
        contact_line_id,
    ...
    if (h.isEmpty(trackerEntry)) {
      ...
      line_message_tracker_id = trackerEntry.dataValues.line_message_tracker_id;
    }
